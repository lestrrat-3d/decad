...
// ruledPatchNormalAllow returns wherever its own derivation degenerates — an
...
// ruledPatchNormalAllow bounds how far a cap-loop chamfer's circular band
// patch can carry an outward unit normal differing from the one the `Cone`
// this evaluator publishes for that patch gives at the same point
// (docs/modify-reach-design.md §8.3). It is a DIMENSIONLESS distance between
// two unit vectors, and it is zero exactly where the patch's two directrices
// sweep one shared window — a tangent join, a reflex corner's apex patch, and
// a whole turn — which is what leaves every already-shipped normal reading
// unchanged.
//
// The build RULES the patch with straight `Line3` rulings between a side
// directrix (radius R0 at level z0, angle θS(u) linear across the side
// window) and a cap directrix (radius R1 at z1, θC(u) linear across the
// trimmed cap window), so
//
//	P(u, v) = (1-v)·A(u) + v·B(u),  A on the side arc, B on the cap arc,
//	Pu × Pv = H·w + s·ẑ, H = z1 - z0, e(θ) = (cos θ, sin θ, 0),
//	w = a·e(θS) + b·e(θC),  a = (1-v)·R0·dS,  b = v·R1·dC,
//	s = a·(R0 - R1·cos δ) - b·(R1 - R0·cos δ),  δ = θS(u) - θC(u),
//
// while the published cone's own normal at that same point is proportional to
// H·e(φ) + (R0 - R1)·ẑ with φ the point's own azimuth (topology.go's Cone
// arm). At δ = 0 the two agree exactly — s collapses to (a+b)(R0 - R1) and w
// to (a+b)·e(θS) — so the whole departure is driven by the two windows' skew,
// in two independent parts:
//
//   - AZIMUTH. Both φ and w's own azimuth are arguments of a non-negative
//     combination of e(θS) and e(θC), so each lies between θS(u) and θC(u) and
//     the two differ by at most the widest corner skew σ. Rotating a unit
//     vector about the axis by σ moves it by at most the angle σ, since the
//     rotation leaves its axial component alone and its in-plane part has
//     magnitude at most one.
//   - TILT, read in the (radial, axial) half-plane: the built patch's own
//     (H·|w|, s) against the cone's (H, R0 - R1) scaled by the same (a+b).
//     |w| - (a+b) is at most (a+b)(1 - cos δ)/2 (from 2ab <= (a+b)²/2), and
//     |s - (a+b)(R0 - R1)| = (1 - cos δ)·|a·R1 - b·R0| is at most
//     (1 - cos δ)(a+b)·max(R0, R1) — so the two 2-vectors differ by at most
//     (1 - cos σ)(|H|/2 + max(R0, R1))·(a+b) against a reference of length
//     (a+b)·hypot(H, R1 - R0). The (a+b) factor CANCELS, which is what makes
//     one bound hold over the whole patch rather than per point, and the angle
//     between two vectors is at most the arcsine of their distance over the
//     reference's own length.
//
// Angles compose along a spherical triangle and a chord never exceeds its own
// arc, so σ plus that arcsine bounds the distance between the two unit
// normals. Where the arcsine's own argument reaches one the tilt is not
// bounded away from a right angle at all and the derivation gives nothing:
// the return is the trivial 2 rather than a clamped value that would read as a
// proof.
func ruledPatchNormalAllow(sideRadius, capRadius, height, windowSkewMax float64) float64 {
	if windowSkewMax <= 0 {
		return 0
	}
	if isNonFinite(windowSkewMax) || isNonFinite(sideRadius) || isNonFinite(capRadius) || isNonFinite(height) {
		return ruledNormalAllowUnbounded
	}
	if windowSkewMax >= math.Pi {
		return ruledNormalAllowUnbounded
	}
	// 1 - cos(x) = 2·sin(x/2)², the numerically stable form chordLocusVolumeAllow
	// already uses: the naive subtraction cancels catastrophically for a small
	// skew, which is exactly the ordinary chamfer's own regime.
	halfSin := math.Sin(windowSkewMax / 2)
	oneMinusCos := upRound(2 * halfSin * halfSin)
	absH := math.Abs(height)
	maxRadius := math.Max(math.Abs(sideRadius), math.Abs(capRadius))
	numer := productUpper(oneMinusCos, absSumUpper(absH/2, maxRadius))
	// The reference length is rounded DOWN so the quotient stays an upper
	// bound of the true ratio whatever hypot committed.
	denom := math.Nextafter(math.Hypot(absH, capRadius-sideRadius), 0)
	if denom <= 0 || isNonFinite(denom) {
		return ruledNormalAllowUnbounded
	}
	ratio := upRound(numer / denom)
	if ratio >= 1 || isNonFinite(ratio) {
		return ruledNormalAllowUnbounded
	}
	total := absSumUpper(windowSkewMax, upRound(math.Asin(ratio)))
	return math.Min(ruledNormalAllowUnbounded, total)
}
