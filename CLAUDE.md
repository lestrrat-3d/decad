...
| `loft_build.go` | `docs/loft-design.md` PR 1a/2a: `loftPayload`, `evalLoft` and `placed` — Table P pairing, the Table S gates it owns (S12 included), the flat-triangle wall/cap construction, the placement re-lift and its `delta`, and the four measurements. See doc comments; `docs/loft-design.md` §5.1/§5.2, §8, §12. |
...
- **Layout rows are pointers, not summaries.** A row states what the file owns
  ...
  an earlier one. And this file admits no HTML comment anywhere: the guard reads
  such a line as part of a table and answers "CLAUDE.md's Layout table did not
  parse", so a note that would be a comment elsewhere is written as prose here.
