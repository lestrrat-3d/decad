...
| `moments.go` | The mass-property engine (evaluator §4): exact Green's-theorem boundary integrals over a `ProfileRecord` — `Area()` (net, Kind Area) and `Centroid()` (plane-local mm) — closed forms for line/circle/arc walks, `ErrUnsupported` for free-form kinds until their increments. |
