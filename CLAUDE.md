...
| `spline_sagitta.go` | `docs/spline-design.md` §6.2.1's chord-segment sagitta bound (`spanSagittaUpper`), `pairStations`'s shared dyadic station generator, and the PARAMETER-MATCHED deviation/speed bounds (`spanMatchedDeltaUpper`/`spanSpeedUpper`) `bounds.go`'s `cellChordCurveAreaUpper` needs instead of the sagitta. |
