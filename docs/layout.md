...
| `verify_result.go` | The private result vocabulary `Verify`'s report is written in: outcome enums, effective-request records, bounded reading wrappers, per-survey/body/report result records. Types only; `verify_publish.go` builds them. |
| `verify_publish.go` | `Verify`'s publication assembler: turns private survey outcomes and certified readings into result records, deciding each survey's outcome, assessment and tolerance state; also the temporary legacy-report bridge. |
