...
| `revolve_build.go` | Builds a revolve's body, solid or (`WithSurfaceResult`) sheet: the wall surface each segment sweeps, the caps a partial sweep closes or omits, the poles and seams a full sweep joins, and the measurements. See evaluator §6, `docs/surface-design.md` §4. |
...
| `verify_result.go` | The result vocabulary `Verify`'s report is written in: `Report`, `BodyReport`, the outcome enums, effective-request records, bounded reading wrappers, and every per-survey result record. Types and the `Passed`/`ForBody` predicates only; `verify_publish.go` builds the values. |
