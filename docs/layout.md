...
| `revolve_denotation.go` | `angleDenotation`/`sweepDenotation`: the exact angle each sweep end denotes, and the proven per-end displacement from it (`phi0Delta`/`phi1Delta`/`angularDelta`) — the angular twin of `prismPayload.z0Delta`/`z1Delta`. See `docs/evaluator-design.md` §6. |
