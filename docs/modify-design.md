...
§13 lands their approved exact proofs as its first two rows. Until then the
same `Suspect` staging applies; a box-disjoint cup pair with no clearance asked
is still proven `Sound` by the box test.
