...
Sweep follows `docs/sweep-design.md` §12's count-free delivery plan. Its first
build increments reduce line spans to Extrude and circular spans to Revolve,
then add composite tangent paths and the global contact audit. Tessellation,
free-form profiles, twist, clearance, and surveys remain staged exactly as its
Table D states. An unlanded Sweep build returns `ErrUnsupported`; an unlanded
verification question reads `Suspect`.
