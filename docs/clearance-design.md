...
two-term `absSumUpper(delta, sectionDelta)` those boxes carry (zero only for an
unplaced `LineSeg`-only loft whose every station is PINNED — loft §5,
§5.2), may decide a box-disjoint partition. A distance measured against a held
