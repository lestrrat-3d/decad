...
| T182 | T180's receiver against three revolve tools in turn: one whose axis anchor differs in the last ulp, one whose angular span stops inside the receiver's, and one that is `Document.Split`'s target rather than `Trim`'s | `ErrUnsupported` (R29) in every case, each read by MESSAGE TEXT and not by `errors.Is` alone, so each is pinned to its own gate: S4 names the axis, S6 names the angular span, and RS13 names `Split`'s staged revolve arm. The document and every operand are unchanged in all three. Shown-to-fail: loosening S4's stored-float axis comparison to an angle comparison admits the last-ulp pair, and the trim it then completes publishes an area whose interval no longer encloses the analytic answer — the leg proving S4's own exactness is load-bearing rather than shadowed by a later gate |
