...
| R31 | `Split` handed a revolve-family pair, or `Extend` handed a partially revolved ribbon — the two arms §1.2 still stages, stated in full as `docs/surface-intersection-design.md` §6's RS13 and RS14 | `ErrUnsupported` |
...
R29 and R31 are `ErrUnsupported` on the same distinction, and R30 joins
R5, R7 and R9 as `ErrDegenerate`: a tool that separates nothing names no
trimmed body for any later evaluator to build. R31's two arms are STAGED —
`docs/surface-intersection-design.md` §11's own Split increment lifts RS13, and
a recorded free-end map from the chain-revolve build lifts RS14 (that
document's §2.2) — while R29 carries that document's §4 set, whose
pair-sharing-no-generator row is permanent.

R22, R34, R36 and R45 are `ErrUnsupported` and STAGED: each names real
...
R37, R38 and R39 take `ErrUnsupported` for the reasons R24, R26 and R27 take
it: a receiver family this evaluator holds no recorded generator for, a
construction it cannot prove, and a value it cannot generate exactly are each
this evaluator's reach rather than geometry with
...
- **`Document.Patch` has no chain-fed form, permanently.** A planar fill needs a
  ...
  proof changes it.
- **`SweepChain` and `LoftChain` each build part of their reach and refuse the
  rest with `ErrUnsupported`**, on §14's own staging rule — the signature lands
  so a caller's intent has somewhere to go and a refusal to read, and each
  remaining build lands with its own increment. `SweepChain` sweeps one straight
  path span, refusing an arc span and every composite path at R34; `LoftChain`
  rules a `LineSeg`-only correspondence, refusing a curved one at R36 and a
  non-parallel plane pair at R35. Each
...
| T96 | a 100×60×40 mm axis-aligned box `Placed` under a rotation about the normalized (1, 2, 3) composed with a 500 mm translation (`vertex_placement_test.go`'s `TestVertexPlacedPrismBoundEnclosesDisplacement`) | each of the eight rim vertices reads `Vertex.Position()` `Approximate` over a strictly positive `Bound`, and the L∞ residual between the held value and the corner the record, frame and placement denote — computed entirely over `math/big.Rat` and rounded UP into a float64, never a second float answer — sits inside that bound. At least one vertex is genuinely displaced, and no bound is compared against a literal. Shown-to-fail: deleting `prism_build.go`'s `frameLiftAllow` term in `buildLoopSidesAs` turns every vertex `Exact` at a zero bound and the `Approximate`/positive assertions go red |
| T97 | the same 100×60 rectangle extruded 40 mm with NO placement, drawn on a tilted, non-origin sketch plane — origin (7, −3, 11), normal the normalized (1, 2, 3) (`TestVertexTiltedPlaneUnplacedBoundEnclosesDisplacement`) | the same reading under the identity transform: lifting a plane-local coordinate through a non-axis-aligned frame rounds with no placement in play, so each rim vertex is `Approximate` over a strictly positive bound enclosing its exact-rational truth. Shown-to-fail: gating `bounds.go`'s `frameAndPlacementRoundAllow` on a non-identity `xform` alone — the loft/stitch/patch/unstitch pattern, which never has to consider its own frame lift — turns this row red while T100 stays green, so this is the row that isolates the frame lift from the placement |
| T98 | `annularSketch` revolved a quarter turn `Along` about `uAxis`, then `Placed` under a rotation about the normalized (1, 2, 3) (`TestVertexPlacedRevolveBoundEnclosesDisplacement`) | the four cap vertices at φ = 0 read `Approximate` over a strictly positive, enclosing bound. The sweep starts where sine and cosine evaluate to exactly (1, 0), so the row charges the frame lift and the placement alone and not the angular denotation `revolve_bounds_test.go` already covers. Shown-to-fail: deleting `revolve_build.go`'s `revolveVertexFrameLiftAllow` term turns the assertions red |
| T99 | a 100×60×20 mm box `Chamfer`ed 5 mm on its complete end-cap loop, then `Placed` under T96's own motion (`TestVertexPlacedCapBlendBoundEnclosesDisplacement`) | the four cap-level corners read `Approximate` over a strictly positive, enclosing bound. A 5 mm chamfer sets each corner back exactly 5 mm along both adjacent edges, so the cap-level corners are the plane-local (5, 5), (95, 5), (95, 55) and (5, 55) at z = 20 by construction and the truth needs no offset-solve reproduction. Shown-to-fail: deleting `capblend_geom.go`'s `frameLiftAllow` term in `buildCapBand` turns the assertions red |
| T100 | the same prism, revolve and cap-blend shapes built axis-aligned and unplaced (`TestVertexAxisAlignedUnplacedBoundStaysZero`) | every vertex of all three bodies reads `Exact` with a `Bound` of exactly zero: the charge the four rows above pin must not widen the common, exact-arithmetic case. Shown-to-fail: dropping `frameAndPlacementRoundAllow`'s own trivial-frame gate, so it charges whenever `maxInputAbs` is nonzero, turns every one of these assertions red |
...
| T203 | radius-10 mm whole-circle prism sheet over 10 mm, offset positive 2 mm and negative 2 mm | one wall face each; `Area` intervals enclose 240π and 160π mm²; the boxes reach ±12 and ±8 mm in x and y and keep z ∈ [0,10] mm |
...
| T205 | T202's result offset again by 1 mm, and T203's positive result offset again by 1 mm | R37 for the first — its section carries corner arcs and is no longer the axis-parallel line class — and a radius-13 mm sheet for the second, `Area` enclosing 260π mm². Shown-to-fail: remove the arc perimeter's rational π enclosure and the radius-13 reading's remaining bound no longer covers its own float evaluation error, turning the 260π enclosure red. That radius is the group's tightest — T202's and T203's own bounds keep enough slack to survive the same deletion, so this row, not T203, is what isolates the enclosure |
...
| T180 | the closed rectangle meridian r ∈ [3, 5], z ∈ [0, 20] revolved a full turn under `WithSurfaceResult` — a closed tube sheet of area 352π mm² — `Trim`med `KeepOutside` by a solid revolve of the rectangle meridian r ∈ [2, 8], z ∈ [5, 10] about the identical axis on the identical frame | `Kind() == BodySheet`; 2 lumps, 3 faces each; `Edges(Free()).Exactly(4)`, the two circles each surviving walk's own free ends sweep (Table G's full-revolution row); `Area` `Approximate` with a strictly positive `Bound` over an interval enclosing 272π mm²; `Volume()` is `ErrNotSolid`; `Tessellate` returns `ErrUnsupported` until increment 4, as every revolve sheet's mesh does. The receiver's section must CLOSE: `docs/surface-intersection-design.md` §2.2 gives `Trim` a closed-loop receiver in both families, and §4's RS3 row refuses a ribbon because `sketch` prunes a dangling spur, publishing an inside stub and an outside stub identically. Shown-to-fail: forcing the result's `sectionDelta` to zero drops the area bound from 3.7e-12 to 1.3e-13 mm² and turns the measured-floor assertion on it red, and publishes the box `Exact` at a zero bound, turning both of its assertions red. The area ENCLOSURE stays green under that forcing — the noise bound still covers the 1.1e-13 mm² the held sum sits from the analytic answer — so the floor assertion is the leg proving §7.1's fold reaches the revolve's own Pappus moment, and the enclosure is what proves the charge is not covering a wrong answer |
...
| T182 | T180's receiver against four revolve tools in turn: one whose axis anchor differs in the last ulp, one about a PARALLEL axis 2 mm away, one whose angular span stops inside the receiver's, and one that is `Document.Split`'s target rather than `Trim`'s | `ErrUnsupported` in every case — R29 for the three entry-gate tools and R31 for the `Split` target — each read by MESSAGE TEXT and not by `errors.Is` alone, so each is pinned to its own gate: S4 names the axis, S6 names the angular span, and RS13 names `Split`'s staged revolve arm. The document and every operand are unchanged in all three. Shown-to-fail: loosening S4's stored-float axis comparison to an angle comparison admits BOTH the last-ulp pair and the parallel-axis pair, and no later gate refuses either. The parallel pair then completes a trim publishing 854.513 mm² — the answer for a genuinely CO-AXIAL tool — for a tool that is not co-axial at all, so the loosened gate publishes a wrong answer rather than a wider bound. The last-ulp pair alone would not show it, a nudge that small sitting under every bound this fixture publishes, so the parallel case is the leg proving S4's exactness load-bearing rather than shadowed by a later gate |
| T183 | the radius-3 mm cylinder band over the line meridian from (3, 0) to (3, 100) recorded over `t` ∈ [0, 0.4] and spun a FULL turn about the same axis — 240π mm² — `Extend`ed along its own carrier against a co-axial revolve solid of the meridian rectangle r ∈ [2, 8], v ∈ [60, 80] | the nearer of the tool's two crossings wins and the band becomes v ∈ [0, 60] — `Area` `Approximate` over an interval enclosing 360π mm², `Bounds` reaching v = 60, and the UNTOUCHED v = 0 rim's position byte-identical to the pre-extend body's, read by ELIMINATION against the rim that moved. Four refusals beside it, each read by message text: a tool whose two crossings lie INSIDE the band's own covered range is RS4 naming the carrier's domain; a PARTIALLY revolved receiver is RS14 naming the full-revolution restriction (`docs/surface-intersection-design.md` §2.2); a tool about a parallel axis 2 mm away is S4 naming the axis; and T180's closed tube sheet is refused for having no open meridian to lengthen. Shown-to-fail: forcing the result's `sectionDelta` to zero drops the area bound from 3.6e-12 to 2.1e-13 mm² and the box bound from 3.6e-13 to 7.1e-15 mm, turning both measured-floor assertions red |
