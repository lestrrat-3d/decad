...
func TestSheetRefusedByBooleanAndModifyOperations(t *testing.T) {
	...
	t.Run("union solid first", func(t *testing.T) {
		...
		doc, sheet, solid := newSheetAndSolid(t)
		before := doc.Bodies()
		_, err := decad.Union(t.Context(), solid, sheet)
		require.ErrorIs(t, err, decad.ErrUnsupported)
		require.Equal(t, before, doc.Bodies())
	})
	t.Run("cut", func(t *testing.T) {
		...
		doc, sheet, solid := newSheetAndSolid(t)
		before := doc.Bodies()
		_, err := decad.Cut(t.Context(), solid, sheet)
		require.ErrorIs(t, err, decad.ErrUnsupported)
		require.Equal(t, before, doc.Bodies())
	})
	t.Run("intersect", func(t *testing.T) {
		...
		doc, sheet, solid := newSheetAndSolid(t)
		before := doc.Bodies()
		_, err := decad.Intersect(t.Context(), sheet, solid)
		require.ErrorIs(t, err, decad.ErrUnsupported)
		require.Equal(t, before, doc.Bodies())
	})
	t.Run("fillet", func(t *testing.T) {
		...
		before := doc.Bodies()
		_, err := sheet.Fillet(t.Context(), decad.Edges(), units.Millimeters(1))
		require.ErrorIs(t, err, decad.ErrUnsupported)
		require.Equal(t, before, doc.Bodies())
	})
	t.Run("chamfer", func(t *testing.T) {
		...
		doc, sheet, _ := newSheetAndSolid(t)
		before := doc.Bodies()
		_, err := sheet.Chamfer(t.Context(), decad.Edges(), units.Millimeters(1))
		require.ErrorIs(t, err, decad.ErrUnsupported)
		require.Equal(t, before, doc.Bodies())
	})
	t.Run("shell", func(t *testing.T) {
		...
		doc, sheet, _ := newSheetAndSolid(t)
		before := doc.Bodies()
		_, err := sheet.Shell(t.Context(), decad.Faces(), units.Millimeters(1))
		require.ErrorIs(t, err, decad.ErrUnsupported)
		require.Equal(t, before, doc.Bodies())
	})
}
...
func TestSheetSolidPairContainedProvenSound(t *testing.T) {
	...
	require.NoError(t, err)
	sheet, err := raw.Placed(t.Context(), shift)
	require.NoError(t, err)
