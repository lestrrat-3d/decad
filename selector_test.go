...
func featureRefWithRole(t *testing.T, body *decad.Body, role string) decad.FeatureRef {
	t.Helper()
	...
	t.Fatalf("body has no feature role %q", role)
	return decad.FeatureRef{}
}
...
func TestCapHelpers(t *testing.T) {
	...
	t.Run("CapStartAndCapEndNameTheRightFaces", func(t *testing.T) {
		require.Equal(t, roleCapStart, decad.CapStart(body).Role)
		require.Equal(t, roleCapEnd, decad.CapEnd(body).Role)
...
func TestSelectorResolutionOnRevolvedBody(t *testing.T) {
	...
	_, err = decad.Edges(decad.CreatedBy(decad.CapStart(body))).SelectEdges(body)
...
func TestSelectorResolutionIsDeterministic(t *testing.T) {
	t.Parallel()
	// The result keeps the topology accessors' order: two resolutions return
...
func TestSelectorPredicateParameterGates(t *testing.T) {
	...
	t.Run("FeatureRef", func(t *testing.T) {
		for _, ref := range []decad.FeatureRef{{Role: ""}} {
